...
int ret = scanf("%d%d", &N, &M); if (ret < 2) break;
for (int c = 0; c < 2; c++) {
v[c].clear();
...
bool play(vector<long long> t[2], bool force) { int p = 0;
while (t[0].size() && t[1].size()) {
if (t[p].back() > t[1-p].back() && !force) { t[1-p].pop_back(); // hostile
} else if (t[p].size() > 1) { t[p][t[p].size() - 2] += t[p].back(); t[p].pop_back(); // friendly
    }
    force = false;
    p = 1-p;
}
